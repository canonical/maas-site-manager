...
from msm.common.token_cache import AccessTokenValidationCache
...
from msm.apiserver.exceptions.constants import (
    ExceptionCode,
)
...
class OAuth2Client:
    ...
    async def callback(self, code: str, nonce: str) -> OAuthCallbackData:
        try:
            ...
            logger.info("Authentication login unsuccessful")
            raise
            ...
            raise UnauthorizedException(
                ...
                details=[
                    BaseExceptionDetail(
                        ...
                        messages=[
                            f"Userinfo 'sub' claim '{response.get('sub')}' does not match ID token 'sub' claim '{id_token_claims.get('sub')}'"
                        ],
                    )
                ],
            )
    ...
    async def validate_access_token(
        ...
        """
        if self.provider.token_type == OIDCProviderAccessTokenType.JWT:
        ...
        try:
            ...
            raise UnauthorizedException(
                code=ExceptionCode.INVALID_TOKEN,
                message="Access token validation failed: no introspection endpoint available, and userinfo endpoint validation failed",
                details=[
                    BaseExceptionDetail(
        ...
        try:
            ...
            raise BadGatewayException(
                ...
                details=[
                    BaseExceptionDetail(
                        reason=ExceptionCode.PROVIDER_COMMUNICATION_FAILED,
                        messages=["Failed to fetch tokens from OIDC server."],
                    )
                ],
            ) from e
            ...
            raise BadGatewayException(
                ...
                details=[
                    BaseExceptionDetail(
                        reason=ExceptionCode.PROVIDER_COMMUNICATION_FAILED,
                        messages=[
                            "Failed to introspect access token with OIDC server."
                        ],
                    )
                ],
            )
        ...
        if not result.get("active", False):
            ...
            raise UnauthorizedException(
                ...
                details=[
                    BaseExceptionDetail(
                        reason=ExceptionCode.INVALID_TOKEN,
                        messages=["Access token is not active."],
                    )
                ],
            )
        ...
        raise BadGatewayException(
            ...
            details=[
                BaseExceptionDetail(
                    reason=ExceptionCode.MISSING_PROVIDER_CONFIG,
                    messages=[
                        "Userinfo endpoint is not configured for the provider."
                    ],
                )
            ],
        )
        ...
        try:
            ...
            raise BadGatewayException(
                ...
                details=[
                    BaseExceptionDetail(
                        reason=ExceptionCode.PROVIDER_COMMUNICATION_FAILED,
                        messages=["Failed to retrieve JWKS from OIDC server."],
                    )
                ],
            ) from e
